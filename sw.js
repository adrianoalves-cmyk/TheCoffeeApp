// ══════════════════════════════════════════════════
//  Brew Manager — Service Worker
//  Handles offline caching so the app shell loads
//  even without an internet connection.
//  Data sync (Google Sheets) still needs internet.
// ══════════════════════════════════════════════════

const CACHE_NAME = 'brew-manager-v1';
...
self.addEventListener('fetch', event => {
  ...
  if (url.hostname === 'script.google.com' || url.hostname === 'fonts.gstatic.com') {
    event.respondWith(
  ...
  // Cache-first for everything else (app shell, fonts CSS)
  event.respondWith(
    caches.match(event.request).then(cached => {
      ...
      return fetch(event.request).then(response => {
        ...
        if (response && response.status === 200 &&
            (url.origin === self.location.origin || url.hostname.includes('googleapis.com') || url.hostname.includes('gstatic.com'))) {
